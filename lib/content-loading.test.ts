  ...
  it("validates", async () => {
    ...
    for (const content of Array.from(allContent.values())) {
      for (const rest of content.rest) {
        ...
        expect(
          version,
          `Invalid version in REST request "${rest.url}"`
        ).toMatch(/v[12]/);
        ...
        let found = false;
        for (const [def, pathSchema] of Object.entries(
          version === "v1" ? schema.paths : schema.paths
        )) {
          if (path === def) {
            found = true;
            ...
            for (const param of schema.parameters) {
              if (
                param.in === "query" &&
                param.required &&
                !reqURL.searchParams.has(param.name)
              ) {
                throw new Error(
            ...
            for (const param of Array.from(reqURL.searchParams)) {
              if (!schema.parameters.find((p) => p.name === param[0])) {
                throw new Error(
                  `Unknown parameter ${param[0]} in REST request "${rest.url}"`
                );
              }
            }
            ...
            if (rest.body) {
              try {
                ...
                throw new Error(
                  `Invalid JSON body in REST request "${rest.url}": ${e}`
                );
              }
            }
        ...
        expect(
          found,
          `Invalid path in REST request "${rest.url}"`
        ).toBeTruthy();
      }
    }
  });
